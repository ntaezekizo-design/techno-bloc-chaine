...
router.post('/mine', async (req, res) => {
  try {
    ...
    const blockchain = new Blockchain();
    const result     = await blockchain.mineNextBlock(minerAddress);
...
// GET /api/mine — kept for quick browser test only, prefer POST
...
router.post('/transaction', async (req, res) => {
  try {
    ...
    const validation = await Transaction.validate(from, to, amount, fee);
