...
class Blockchain {
  ...
  async adjustDifficulty(currentHeight) {
    ...
    const elapsed    = parseInt(row.last_time) - parseInt(row.first_time);
  ...
  async getStats() {
    ...
    const [wallets] = await query('SELECT COUNT(*) AS c FROM wallets');
    const [mempool] = await query('SELECT COUNT(*) AS c FROM mempool');
    stats.total_wallets   = parseInt((wallets || {}).c) || 0;
    stats.mempool_count   = parseInt((mempool || {}).c) || 0;
    const supply = parseFloat(stats.total_supply) || 0;
    const maxSup = require('../config/blockchain').MAX_SUPPLY;
    stats.circulation_pct = maxSup > 0 ? Math.round((supply / maxSup) * 100 * 10000) / 10000 : 0;
    return stats;
  }
